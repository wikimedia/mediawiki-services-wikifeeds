 ...
 *
 *  @param {!Object} result result page from MW API
 *  @return {!number} estimated article quality score
 */
 ...
 * @return {!Promise} promise resolving to the requested feed content
 */
function promise(app, req) {
    ...
    return BBPromise.join(
        ...
        (siteinfo, queryResponse) => {
            util.checkForQueryPagesInResponse(req, queryResponse);
            ...
            return {
                title: util.getDbTitle(bestResult.title, siteinfo),
                meta: {
                    etag: util.getRevisionFromExtract(bestResult)
                }
            };
        });
}
