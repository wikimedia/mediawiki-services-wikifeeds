...
const languages = {
    ...
    el: {
        ...
        dayPage: {
            // https://el.wikipedia.org/api/rest_v1/page/html/16_Ιανουαρίου
            nameFormatter: (monthName, monthNumber, dayNumber) => `${dayNumber}_${monthName}}`,
            headingIds: {
                births: [ 'Γεννήσεις' ],
                deaths: [ 'Θάνατοι' ],
                events: [ 'Γεγονότα' ],
                holidays: [ 'Αργίες_και_εορτές' ]
            }
        },
        selectedPage: {
            // https://el.wikipedia.org/api/rest_v1/page/html/Πύλη:Κύρια%2FΕπιλεγμένα_Γεγονότα%2F16_Ιανουαρίου
            nameFormatter: (monthName, monthNumber, dayNumber) =>
                `Πύλη:Κύρια/Επιλεγμένα_Γεγονότα/${dayNumber}_${monthName}`,
            listElementSelector: 'body > ul li,section > ul li'
        },
        yearListElementRegEx:
          new RegExp(String.raw`^\s*(\d+)\s*(π\.\s*Χ\.)?\s*(?::|[${dashChars}])\s*(.*\S.*)`, 'i'),
        yearPrefixRegEx:
          new RegExp(String.raw`^\s*(\d+)\s*[:${dashChars}]*\s*$`, 'i')
    },
