...
function buildMeta(page) {
    ...
    return {
        ...
        tid: util.getRevisionFromExtract(page)
    };
}
 ...
 * @param {?Array} ids query results for Commons page ids (for media endpoint)
 * @param {!Array} meta metadata query results to be parsed and formatted
...
function makeResults(ids, meta, siteinfo, preferredLang, desc = null) {
    ...
    return meta.map((item) => {
        ...
        const isSvg = imageInfo.mime && imageInfo.mime.includes('svg'); // eslint-disable-line no-restricted-syntax
        const canonicalTitle = util.getDbTitle(item.title, siteinfo);
...
function requestPictureOfTheDay(app, req, date, siteinfo) {
    ...
    return api.mwApiGet(app, COMMONS_URL, query).then((response) => {
        util.checkForQueryPagesInResponse(req, response);
 ...
 * @param {!Array} titles page titles to request
 * @param {!Object} siteinfo siteinfo object
 * @return {!Promise} promise to get metadata for the titles from the API
 */
function getMetadataFromApi(app, req, titles, siteinfo) {
    ...
    return P.join(
        api.mwApiGetBatched(app, 'commons.wikimedia.org', { action: 'query' }, titles.slice(0), req.headers),
